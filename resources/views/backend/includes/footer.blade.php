            ...
            <div class="col-md-6">
                <p class="clearfix blue-grey lighten-2 text-sm-center mb-0 px-2 footer-text">
                    <span class="float-md-left d-block d-md-inline-block ">
                        ...
                        <a class="text-bold-800 grey darken-2" href="{{ route('admin.dashboard') }}" target="_blank">
                            JMBAXI GROUP</a>
                    </span>
                </p>
            </div>
...
<script src="{{ asset('public/backend-assets/js/vendor.bundle.base.js') }}"></script>
<script src="{{ asset('public/backend-assets/js/template.js') }}"></script>
<script src="{{ asset('public/backend-assets/js/Chart.min.js') }}"></script>
<script src="{{ asset('public/backend-assets/js/progressbar.min.js') }}"></script>
<script src="{{ asset('public/backend-assets/js/chartjs-plugin-datalabels.js') }}"></script>
<script src="{{ asset('public/backend-assets/js/justgage.js') }}"></script>
<script src="{{ asset('public/backend-assets/js/raphael-2.1.4.min.js') }}"></script>
...
{{-- Bootstrap - tagsinput --}}
<script src="{{ asset('public/backend-assets/assets/js/bootstrap-tagsinput.js') }}"></script>
...
<script>
    $(document).ready(function() {
        ...
        var data = "{{ isset($roles_external) ? json_encode($roles_external, JSON_HEX_TAG) : '' }}";
    ...
    $(document).ready(function() {
        ...
        function append_clear_button() {
            $('#notifications_lists').append(`<div class="col-sm-12 px-2 text-right clear_notification_btn_div text-end">
                <a href="{{ url('/') }}/admin/notifications/show" class="btn btn-primary btn-sm btn_show_notif" style='float-right;' data="{{ Auth::user()->admin_user_id }}">Show All</a>
                <span class="btn btn-info btn-sm  btn_clear_notif" style='float-right;' data="{{ Auth::user()->admin_user_id }}">Clear</span>
        </div>`);
        }
        ...
        function getNotifications() {
            ...
            $.ajax({
                type: 'GET',
                url: '{{ url('admin/ideas/ajax_get_notifications') }}',
                        ...
                        for (var i = 0; i < jsonData.response.length; i++) {
                            if (return_data[i].status == 'success') {
                                ...
                                var url_update_notification =
                                    '{{ url('admin/ideas/ajax_update_notification/:notification_id') }}/' +
                                    idea_id;
...
{{-- usama_09-02-2024 -- for fetching category data dynamically --}}
<script>
    function get_dynamic_categories(type_id) {
        if (type_id != 'User') {
            document.querySelector('.external_user_cat_data').classList.remove("d-none");
            var html = '';
            var idea_cat_arr = {!! isset($categories) ? json_encode($categories) : '[]' !!};
            html += `
          <h4 class="card-title">
...
<script>
    ...
    $('#NavBar').on('click', '.btn_clear_notif', function(e) {
        ...
        let user_role = "{{ isset($roles_external->role_name) ? $roles_external->role_name : '' }}";
        ...
        if (confirm('do you want to clear all notification?')) {
            $.ajax({
                type: 'post',
                url: '{{ url('admin/ideas/ajax_clear_notification') }}',
                data: data,
...
<script>
    ...
    let timer = setTimeout(() => {
        ...
        "{{ url('/') }}/admin/logout/timeout"; // Change 'logout.html' to the actual logout page URL
    }, logoutTime);
    ...
    window.addEventListener('beforeunload', function (event) {
        ...
        fetch("{{ url('/') }}/admin/logout/timeout", {
            method: 'POST',
            headers: {
