...
<header class="navbar navbar-expand-md d-print-none cs-header">
    <div class="container-fluid">
        ...
        <a href="{{ url('/') }}/user/dashboard"class="navbar-brand navbar-brand-autodark d-none-navbar-horizontal pe-0 pe-md-3">
            ...
            <div class="nav-item dropdown d-md-flex me-3">
                ...
                <div class="dropdown-menu dropdown-menu-end dropdown-menu-card">
                    ...
                    <div class="col-sm-12 px-2 text-right clear_notification_btn_div text-end">
                        <a href="{{ url('/') }}/user/notifications/show" class="btn btn-primary btn-sm btn_show_notif m-1 " style='float-right;' data="{{ Auth::user()->user_id }}">Show All</a>
                        <span class="btn btn-info btn-sm  btn_clear_notif m-1" style='float-right;' data="{{ Auth::user()->user_id }}">Clear</span>
                    </div>
            ...
            <div class="nav-item dropdown">
                <a class="dropdown-toggle nav-link dropdown-user-link" href="#" data-toggle="dropdown">
                    <div class="avatar avatar-online">
                        <img src="{{ asset('/public/frontend-assets/images/user.png') }}" alt="avatar">
                    </div>
                    <span class="user-name ps-2">{{ $user->name }} {{ $user->last_name }} <i>{{ $user->designation }}</i></span>
                </a>
            ...
            <div class="container-fluid">
                @if (in_array('menu_values', $menus))
                    <ul class="navbar-nav">
                        ...
                        @if (in_array('all ideas',$menus))
                        <li class="nav-item {{$all_idea_active}}">
                            <a class="nav-link" href="{{url('/')}}/all/ideas">
                                <span class="nav-link-icon d-md-none d-lg-inline-block">
                        ...
                        @if (strtolower($roles_external->role_type) == 'user')
                            @if (in_array('ideas_for_approval', $menus))
                                <li class="nav-item {{ $ideaManagement_active }}">
                                    <a class="nav-link" href="{{ url('/') }}/ideas">
                                        <span class="nav-link-icon d-md-none d-lg-inline-block">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24"
                        ...
                        @if (!$curr_user_data->isEmpty() && $roles_external->role_type != 'User' && in_array('my_ideas', $menus))
                            <li class="nav-item {{ $ideaManagement_active_myideas }}">
                                <a class="nav-link" href="{{ url('/myideas') }}">
                                    <span class="nav-link-icon d-md-none d-lg-inline-block">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                            <path d="M14 3v4a1 1 0 0 0 1 1h4" />
                        ...
                        @if (!$curr_user_data->isEmpty() && in_array('rewards', $menus))
                            <li class="nav-item {{ $rewards_active }}">
                    ...
                    <div class="status">Logged in as : 
                        <strong>{{ $roles_external->role_name }}</strong>
                    </div>
                @endif
            </div>
        </div>
