...
<script src="{{ asset('public/frontend-assets/static/js/app.js') }}"></script>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function getNotifications() {
            ...
            $.ajax({
                type: 'GET'
                , url: '{{ url('ideas/ajax_get_notifications') }}'
                , success: function(data) {
                    ...
                    if (jsonData.response.length > 0) {
                        ...
                        for (var i = 0; i < jsonData.response.length; i++) {
                            ...
                            var url_update_notification = '{{ url("ideas/ajax_update_notification/:notification_id/:idea_id") }}';
    ...
    window.location = "{{ url('/users/updaterole') }}/" + role;
    ...
    $('#reset_file').click(function(){
       ...
       let user_role = "{{ (isset($roles_external->role_type)?$roles_external->role_type:'') }}";


       let data = {'_token':"{{ @csrf_token() }}", 'id': user_data_id,'user_role':user_role };
       if(confirm('do you want to clear all notification?')){
        $.ajax({
            type: 'post'
            , url: '{{ url('ideas/ajax_clear_notification') }}'
            , data: data
    ...
    let timer = setTimeout(() => {
        ...
        "{{ url('/') }}/user/logout/timeout"; // Change 'logout.html' to the actual logout page URL
    }, logoutTime);
