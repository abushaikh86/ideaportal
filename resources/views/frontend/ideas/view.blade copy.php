                                                ...
                                                @if (count($idea_status) > 0)
                                                    <li class="divider">
                                                        <button type="button" class="btn btn-primary"
                                                            data-bs-toggle="modal" data-bs-target="#statusTimelineModal">
                                        ...
                                        @else
                                            @if (isset($idea->assessment_team_approval) &&
                                                ...
                                                <div class="card status-card mt-4">
                                                    ...
                                                    <div id="card-body">
                                                        {{ Form::open(['method' => 'POST', 'route' => ['ideas.updateIdeaStatus'], 'class' => 'form']) }}
                                                        @csrf
                                                        ...
                                                        <div class="idea-status-box mb-2 mt-3 mx-2">
                                                            ...
                                                            <select class="form-control me-2" id="idea_status"
                                                                name="idea_status">
                                                                ...
                                                                @foreach ($idea_active_status as $key => $status_option)
                                                                    ...
                                                                    <option
                                                                        {{ $idea['active_status'] == $key ? 'selected' : '' }}
                                                                        {{ $key == 'pending' ? 'disabled' : '' }}
                                                                        value="{{ $key }}">{{ $status_option }}
                                                                        {{ $reason }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                        ...
                                                        <div id="content1" style="display:flex;height:50px;"
                                                            class=" d-flex">
                                                            <input type="text" id="reason_resubmit"
                                                                name="resubmit_reason"
                                                                class="form-control w-100 mb-1 mt-1"
                                                                placeholder="Enter you reason here"
                                                                @if (isset($idea->resubmit_reason) && !is_null($idea->resubmit_reason)) value="{{ $idea->resubmit_reason }}" @endif><br>

                                                            <input type="text" id="reason_reject" name="reject_reason"
                                                                class="form-control w-100 mb-1 mt-1"
                                                                placeholder="Enter you reason here"
                                                                @if (isset($idea->reject_reason) && !is_null($idea->reject_reason)) value="{{ $idea->reject_reason }}" @endif><br>
                                                        </div>
                                                        <div id="content" style="display:flex;height:50px;"
                                                            class=" d-flex">
                                                            <input type="text" id="reason_sla" name="sla_reason"
                                                                class="form-control w-100 mb-1 mt-1"
                                                                placeholder="Enter you reason for SLA not meet"
                                                                @if (isset($idea->sla_reason_assessment) && !is_null($idea->sla_reason_assessment)) value="{{ $idea->sla_reason_assessment }}" @endif>
                                                        </div>
                                                        {{ Form::close() }}
                                                    </div>
                                                </div>
                                                        ...
                                                        @if ($assessment_team_approval_flag == 0)
                                                            <div class="d-flex justify-content-end mt-3">
                                                                <button type="button" class="btn btn-success"
                                                                    data-toggle="modal" data-target="#approveModal">Submit
                                                                    for
                                                                    Approval</button>
                                                            </div>
                                                        @endif
                                ...
                                <div class="list">
                                    <div class="item">
                                        ...
                                        <div class="content">
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                ...
                                                {{--  {{ Form::select('assessment_idea_clarity', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->idea_description) ? $journey_assesment->idea_description : null, ['class' => 'form-select d-none', 'rows' => '4', 'required' => true, 'id' => 'assessment_idea_clarity', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="content">
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                <div class="action-btn">
                                                    ...
                                                    <button class="like-btn {{isset($journey_assesment->outcome) &&  $journey_assesment->outcome == 'like' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }} data-name='assessment_outcome'
                                                        data-btn-name='like'>
                                                        <i class="fa fa-thumbs-up"></i>
                                                    </button>
                                                    <button class="dislike-btn {{isset($journey_assesment->outcome) &&  $journey_assesment->outcome == 'dislike' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }} data-name='assessment_outcome'
                                                        data-btn-name='dislike'>
                                                        <i class="fa fa-thumbs-down"></i>
                                                    </button>
                                                </div>
                                                {{--  {{ Form::select('assessment_outcome', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->outcome) ? $journey_assesment->outcome : null, ['class' => 'form-select d-none', 'rows' => '4', 'required' => true, 'id' => 'assessment_outcome', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                <div class="action-btn">
                                                    <p>Did you understand why the idea should be implemented and what makes
                                                        it unique?</p>
                                                    {{--  <!-- Add active class to button for selection -->  --}}
                                                    <button class="like-btn {{ isset($journey_assesment->why_implemented) &&  $journey_assesment->why_implemented == 'like' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_why_implemented' data-btn-name='like'>
                                                        <i class="fa fa-thumbs-up"></i>
                                                    </button>
                                                    <button class="dislike-btn {{ isset($journey_assesment->why_implemented) &&  $journey_assesment->why_implemented == 'dislike' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_why_implemented' data-btn-name='dislike'>
                                                        <i class="fa fa-thumbs-down"></i>
                                                    </button>
                                                </div>
                                                {{--  {{ Form::select('assessment_why_implemented', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->why_implemented) ? $journey_assesment->why_implemented : null, ['class' => 'form-select d-none', 'rows' => '4', 'required' => true, 'id' => 'assessment_why_implemented', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="content">
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                <div class="action-btn">
                                                    ...
                                                    {{--  <!-- Add active class to button for selection -->  --}}
                                                    <button class="like-btn {{ isset($journey_assesment->challenges) &&  $journey_assesment->challenges == 'like' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_challenges' data-btn-name='like'>
                                                        <i class="fa fa-thumbs-up"></i>
                                                    </button>
                                                    <button class="dislike-btn {{ isset($journey_assesment->challenges) &&  $journey_assesment->challenges == 'dislike' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_challenges' data-btn-name='dislike'>
                                                        <i class="fa fa-thumbs-down"></i>
                                                    </button>
                                                </div>
                                                {{--  {{ Form::select('assessment_challenges', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->challenges) ? $journey_assesment->challenges : null, ['class' => 'form-select d-none', 'rows' => '4', 'required' => true, 'id' => 'assessment_challenges', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="content">
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                <div class="action-btn">
                                                    ...
                                                    {{--  <!-- Add active class to button for selection -->  --}}
                                                    <button class="like-btn {{ isset($journey_assesment->idea_not_implemented) &&  $journey_assesment->idea_not_implemented == 'like' ? 'active' : '' }} " data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_is_no_implemented' data-btn-name='like'>
                                                        <i class="fa fa-thumbs-up"></i>
                                                    </button>
                                                    <button class="dislike-btn {{ isset($journey_assesment->idea_not_implemented) &&  $journey_assesment->idea_not_implemented == 'dislike' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_is_no_implemented' data-btn-name='dislike'>
                                                        <i class="fa fa-thumbs-down"></i>
                                                    </button>
                                                </div>
                                                {{--  {{ Form::select('assessment_is_no_implemented', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->idea_not_implemented) ? $journey_assesment->idea_not_implemented : null, ['class' => 'form-select d-none', 'rows' => '4', 'required' => true, 'id' => 'assessment_is_no_implemented', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="content">
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                <div class="action-btn">
                                                    ...
                                                    {{--  <!-- Add active class to button for selection -->  --}}
                                                    <button class="like-btn {{ isset($journey_assesment->has_alternatives) &&  $journey_assesment->has_alternatives == 'like' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_has_alternatives' data-btn-name='like'>
                                                        <i class="fa fa-thumbs-up"></i>
                                                    </button>
                                                    <button class="dislike-btn {{ isset($journey_assesment->has_alternatives) &&  $journey_assesment->has_alternatives == 'dislike' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_has_alternatives' data-btn-name='dislike'>
                                                        <i class="fa fa-thumbs-down"></i>
                                                    </button>
                                                </div>
                                                {{--  {{ Form::select('assessment_has_alternatives', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->has_alternatives) ? $journey_assesment->has_alternatives : null, ['class' => 'form-select d-none', 'rows' => '4', 'required' => true, 'id' => 'assessment_has_alternatives', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="content">
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                <div class="action-btn">
                                                    ...
                                                    {{--  <!-- Add active class to button for selection -->  --}}
                                                    <button class="like-btn {{ isset($journey_assesment->has_less_benifits) &&  $journey_assesment->has_less_benifits == 'like' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_has_less_benifits' data-btn-name='like'>
                                                        <i class="fa fa-thumbs-up"></i>
                                                    </button>
                                                    <button class="dislike-btn {{ isset($journey_assesment->has_less_benifits) &&  $journey_assesment->has_less_benifits == 'dislike' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }}
                                                        data-name='assessment_has_less_benifits' data-btn-name='dislike'>
                                                        <i class="fa fa-thumbs-down"></i>
                                                    </button>
                                                </div>
                                                {{--  {{ Form::select('assessment_has_less_benifits', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->has_less_benifits) ? $journey_assesment->has_less_benifits : null, ['class' => 'form-select d-none', 'rows' => '4', 'required' => true, 'id' => 'assessment_has_less_benifits', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="content">
                                            ...
                                            @if (strtolower($roles_external->role_type) == 'assessment team')
                                                <div class="action-btn">
                                                    ...
                                                    <!-- Add active class to button for selection -->
                                                    <button class="like-btn {{ isset($journey_assesment->benifits) &&  $journey_assesment->benifits == 'like' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }} data-name='benifits'
                                                        data-btn-name='like'>
                                                        <i class="fa fa-thumbs-up"></i>
                                                    </button>
                                                    <button class="dislike-btn {{ isset($journey_assesment->benifits) &&  $journey_assesment->benifits == 'dislike' ? 'active' : '' }}" data-idea-id="{{ $idea->idea_id }}"
                                                        data-user-id={{ $idea->user_id }} data-name='benifits'
                                                        data-btn-name='dislike'>
                                                        <i class="fa fa-thumbs-down"></i>
                                                    </button>
                                                </div>
                                                {{--  {{ Form::select('benifits', ['yes' => 'Yes', 'no' => 'No'], isset($journey_assesment->benifits) ? $journey_assesment->benifits : null, ['class' => 'form-select', 'rows' => '4', 'required' => true, 'id' => 'benifits', 'placeholder' => 'Select Value']) }}  --}}
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    @if (strtolower($roles_external->role_type) == 'assessment team')
                                        <div class="item">
                                            ...
                                            <div class="content">
                                                {{ Form::text('remarks', isset($journey_assesment->remarks) ? $journey_assesment->remarks : null, ['class' => 'form-control', 'id' => 'remarks', 'placeholder' => 'Enter Remarks']) }}
                                                @if (isset($disabled) && $disabled != 'disabled')
                                                    <button class="btn btn-primary mt-3"
                                                        id="update_details_assesment">Update
                                                        Details</button>
                                                @endif
                                            </div>
                                        </div>
                                                ...
                                                <div class="col-lg-3 col-md-4 col-sm-6">
                                                    <a style="margin-top:10px;"
                                                        ...
                                                        @if (in_array(strtolower($ext), $ALL_VIDEO_EXTENSIONS))
                                                            <video style="width:100%;height:50px; object-fit:contain"
                                                                controls>
