...
class AccountController extends Controller
{
  ...
  {
    $this->middleware('guest:admin', ['except' => ['logout','logouttimeout']]);
  }
      ...
      if (isset(Auth()->guard('admin')->user()->admin_user_id)) {
        ...
        if (Auth()->guard('admin')->user()->account_status == 0) {
          ...
          return back()->withErrors([
            'message' => 'Your account has been deactivated, Please contact Idea Portal Team to reactivate your account.'
          ]);
        }
      }
...
public function logouttimeout(){
    Auth::guard('admin')->logout();
    return redirect()->route('admin.login')->withErrors(['message'=>'Session Timeout Please login again']);
  }
