...
class ExternalusersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($users)
                ...
                // class="print_checkbox" value="{{ $data->invoice_id }}"
                ...
                ->addColumn('location', function ($users) {
                    ...
                    if (isset($users->location)) {
                        return $location = Location::where('location_id', $users->location)->first()->location_name;
                    } else {
                        return $location = '--';
                    }
                })
                ->addColumn('company_name', function ($users) {
                    return isset(Company::where('company_id', $users->company_id)->first()['company_name']) ? Company::where('company_id', $users->company_id)->first()['company_name'] : '';
                })
                ->addColumn('department', function ($users) {

                    $dname = Department::where('department_id', $users->department)->first();
                    if (isset($users->name)) {

                        return $dname->name;
                    }
                })
                ->addColumn('active_status', function ($users) {
                ...
                ->addColumn('action', function ($users) {
                    ...
                    $btn .= '<a href="'.url('/').'/admin/externalusers/edit/' . $users->user_id . '"
                    class="btn btn-darkblue"><i class="feather icon-edit-2"></i></a>';

                    $btn .= '<a href="'.url('/').'/admin/externalusers/delete/' . $users->user_id . '"
                    class="btn btn-danger" onclick = "return confirm(`Are you sure you want to Delete this Entry ?`)"><i class="feather icon-trash"></i></a>';

                    $btn .= '<a href="'.url('/').'/admin/externalusers/changepassword/' . $users->user_id . '"
        ...
        return view('backend.externalusers.index');
    }
    ...
    public function  updatestatus(Request $request)
    {
        ...
        if (count($data) > 0) {
            ...
            if(!empty($request->category_id)){
                $catgegory_ids = implode(",",$request->category_id);
                $userdata->category_id = $catgegory_ids;
            }
            ...
            if ($userdata->save()) {
                //   dd($userdata->toArray());
                $exits_role = 0;
                if (isset($userdata->sub_role_final) && !is_null($userdata->sub_role_final)) {
                    $last_role = $userdata->sub_role_final;
                    $current_roles = explode(',', $userdata->sub_role);
                    if (isset($current_roles) && count($current_roles) > 0) {
                    ...
                    if ($exits_role == 0) {
                        $userdata->sub_role_final = $request->role[0];
                        $userdata->save();
                    }
                }
    ...
    public function changepassword($id)
    {
        ...
        if ($user) {
            return view('backend.externalusers.update_password', compact('user'));
        } else {
            return redirect()->route('admin.externalusers');
        }
    }
    ...
    {
        //dd($request->all());  //confirm_password
        $request->validate([
        ...
        if ($user) {
            ...
            if ($user->save()) {
                return redirect()->route('admin.externalusers')->with('success', 'Password has been Changed');
            } else {
                return redirect()->route('admin.externalusers')->with('info', 'failed to update password');
            }
        } else {
            return redirect()->route('admin.externalusers')->with('error', 'user not found');
        }
    }
}//end of class
